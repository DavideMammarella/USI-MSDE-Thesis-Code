...
def calculate_results(df):
    return df.groupby('threshold_type', as_index=False).agg(TP=('true_positives', 'sum'),
...
def normalize_dataframe(df):
    return df.drop(columns=['simulation', 'num_normal', 'auroc', 'false_positive_rate', 'pr_auc'])


def get_simulator(csv_file, simulator_name):
    iter_csv = pd.read_csv(csv_file, iterator=True, chunksize=1000)
    df = pd.concat([chunk[chunk['simulation'].str.contains(simulator_name)] for chunk in iter_csv])
    return df
...
def main():
    ...
    for i, name in enumerate(analysis_names, start=1):
        ...
        for sim in sims:
            ...
            print(result_df)
            result_df.to_csv(Path(metrics_path, "summary_analysis_" + str(i) + "_" + sim + ".csv"), index=False)
