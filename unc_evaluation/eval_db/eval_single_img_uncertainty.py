...
class SingleImgUncertainty:
    def __init__(self, setting_id: int, row_id: int, is_crash: bool, uncertainty: float):
...
def load_all_for_setting(db: Database, setting_id: int) -> List[SingleImgUncertainty]:
    cursor = db.cursor.execute('select * from single_image_based_uncertainties where setting_id=? ' +
                               'order by row_id',
                               (setting_id,))
    var = cursor.fetchall()
    result = []
    for db_record in var:
        ...
        uncertainty_object = SingleImgUncertainty(
            setting_id=db_record[0], row_id=db_record[1], is_crash=is_crash, uncertainty=db_record[3])
...
def update_true_label_on_db(db: Database, records: List[SingleImgUncertainty]):
    for record in records:
        ...
        db.cursor.execute('update single_image_based_uncertainties set true_label=?, count_to_crash=? ' +
