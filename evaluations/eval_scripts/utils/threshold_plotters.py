    ...
    def compute_and_plot(self):
        ...
        for i in range(self.data_points):
            ...
            for ad in AD_NAMES.keys():
                ...
                threshold = b_precision_recall_auroc.get_threshold(
                    ad_name=ad, threshold_type=self.threshold_type
                )
                # Temporary for calculations, not stored to db
                analysis = (
                    b_precision_recall_auroc.create_precision_recall_analysis(
                        ad_name=ad,
                        auroc=None,
                ...
                results["TP"][ad].append(
                    self.replace_minus_one(analysis.true_positives)
                )
                results["FP"][ad].append(
                    self.replace_minus_one(analysis.false_positives)
                )
                results["TN"][ad].append(
                    self.replace_minus_one(analysis.true_negatives)
                )
                results["FN"][ad].append(
                    self.replace_minus_one(analysis.false_negatives)
                )
                results["TPR"][ad].append(
                    self.replace_minus_one(analysis.recall)
                )
                results["FPR"][ad].append(
                    self.replace_minus_one(analysis.false_positive_rate)
                )
                results["F1"][ad].append(self.replace_minus_one(analysis.f1))
                results["Prec"][ad].append(
                    self.replace_minus_one(analysis.prec)
                )
