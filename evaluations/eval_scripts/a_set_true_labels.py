...
def set_true_labels(db_path):
    ...
    for setting in settings:
        ...
        single_img_entries = eval_single_img_uncertainty.load_all_for_setting(
            db, setting_id=setting.id
        )
        current_window_count = _set_true_labels(
            entries=single_img_entries,
            current_setting_id=setting.id,
            current_ad_type=current_ad_type,
            current_window_count=current_window_count,
            db=db,
        )
        eval_single_img_uncertainty.update_true_label_on_db(
            db=db, records=single_img_entries
        )
...
def _detect_healing(entries):
    idx_after_misbehavior_series = _first_after_misbehavior_series(
        entries=entries
    )
    for start_index in idx_after_misbehavior_series:
        ...
        for i in range(start_index, end_index_excl):
            ...
            if entry.true_label is None:
                ...
                logger.error(
                    "At this point of the label setting, there should only be misbehaviors and None labels"
                )
                assert False
...
def _mark_window_with_label(
    entries, start_inclusive: int, end_excl: int, label: str
):
...
def _get_window_idx_or_mark_gap(
    start_inclusive, window_size, entries
) -> (bool, int, int):
...
def _detect_and_store_main_blocks(
    entries,
    current_setting_id: int,
    current_ad_type: str,
    window_count: int,
    directly_normal: bool,
    db: Database,
):
    ...
    first_after_misbehaviour_series = _first_after_misbehavior_series(
        entries=entries
    )
    if directly_normal:
        if (
            len(first_after_misbehaviour_series) > 0
            and first_after_misbehaviour_series[0] == MAX_CUT_END_LENGTH + 1
        ):
    ...
    for right_after_misbehaviour_series in first_after_misbehaviour_series:
        if not directly_normal:
            # Set reaction window
            abort, reaction_end_excl = attempt_to_set_reaction_window(
                entries=entries, start_inc=right_after_misbehaviour_series
            )
            if abort:
                continue
            # Set anomaly window
        ...
        window_count = attempt_to_set_normal_windows(
            entries=entries,
            anomaly_end_exc=anomaly_end_exc,
            window_count=window_count,
            setting_id=current_setting_id,
            ad_type=current_ad_type,
            db=db,
...
def _first_after_misbehavior_series(entries) -> List[int]:
    ...
    for i in range(len(entries)):
        ...
        if entry.true_label is None and currently_in_misbehavior_serie:
            ...
            entry.true_label == MISBEHAVIOR_LABEL
            and not currently_in_misbehavior_serie
        ):
            currently_in_misbehavior_serie = True
    return result


def attempt_to_set_normal_windows(
    entries,
    anomaly_end_exc: int,
    window_count: int,
    setting_id: int,
    ...
    stop_point = _next_not_none_label_index(
        entries, anomaly_end_exc, len(entries)
    )
    if stop_point == -1:
    ...
    while end_point_excl <= stop_point:
        _mark_window_with_label(
            entries,
            start_inclusive=start_point,
            end_excl=end_point_excl,
            label=NORMAL_LABEL,
        )
        window_count = _store(
            entries=entries,
            start_inc=start_point,
            end_excl=end_point_excl,
            window_type="normal",
            window_count=window_count,
...
def attempt_to_set_anomaly_window(
    entries,
    start_inc: int,
    window_count: int,
    setting_id: int,
    ...
    no_space, anomaly_start, anomaly_end_excl = _get_window_idx_or_mark_gap(
        start_inclusive=start_inc,
        window_size=ANOMALY_WINDOW_LENGTH,
        entries=entries,
    )
    other_label_found = check_and_treat_not_none_labels(
        anomaly_end_excl=anomaly_end_excl,
    ...
    if not abort:
        _mark_window_with_label(
            entries,
            start_inclusive=anomaly_start,
            end_excl=anomaly_end_excl,
            label=ANOMALY_LABEL,
        )
        window_count = _store(
            entries=entries,
            start_inc=anomaly_start,
            end_excl=anomaly_end_excl,
            window_type="anomaly",
            window_count=window_count,
            setting_id=setting_id,
...
def _store(
    ...
    eval_window.store_single_image_windows(
        setting=setting_id,
...
def attempt_to_set_reaction_window(entries, start_inc):
    no_space, reaction_start, reaction_end_excl = _get_window_idx_or_mark_gap(
        start_inclusive=start_inc, window_size=REACTION_TIME, entries=entries
    )
    other_label_found = check_and_treat_not_none_labels(
        anomaly_end_excl=reaction_end_excl,
        anomaly_start=reaction_start,
        entries=entries,
    )
    abort = no_space or other_label_found
    if not abort:
        _mark_window_with_label(
            entries,
            start_inclusive=reaction_start,
            end_excl=reaction_end_excl,
...
def check_and_treat_not_none_labels(anomaly_end_excl, anomaly_start, entries):
    other_label_in_window_index = _next_not_none_label_index(
        entries=entries,
        start_index_inc=anomaly_start,
    ...
    if other_label_found:
        _mark_window_with_label(
            entries=entries,
            start_inclusive=anomaly_start,
            end_excl=other_label_in_window_index,
            label=LABEL_GAP,
        )
    return other_label_found


def _next_not_none_label_index(
    entries, start_index_inc: int, end_index_excl: int
):
...
def _integrity_checks_after_calc(entries):
    ...
    _check_frame_length(
        entries, label=ANOMALY_LABEL, window_length=ANOMALY_WINDOW_LENGTH
    )
    _check_frame_length(
        entries,
        label=NORMAL_LABEL,
        window_length=NORMAL_WINDOW_LENGTH,
        allow_subsequent_same=True,
    )
    _check_frame_length(
        entries, label=REACTION_LABEL, window_length=REACTION_TIME
    )


def _check_frame_length(
    entries, label, window_length, allow_subsequent_same=False
):
    label_count = 0
    for entry in entries:
...
def _set_true_labels(
    ...
    current_window_count = _detect_and_store_main_blocks(
        entries,
        current_setting_id=current_setting_id,
        current_ad_type=current_ad_type,
