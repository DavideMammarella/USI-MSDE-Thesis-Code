...
if __name__ == "__main__":
    ...
    plt.imshow(
        mpimg.imread(center_images[min_idx_unc]).reshape(
    ...
    plt.title(
        "min uncertainty \n steering angle=%s \n uncertainty=%s"
        % (
            round(online_steering_angles[min_idx_unc], 5),
    ...
    plt.imshow(
        mpimg.imread(center_images[max_idx_unc]).reshape(
            IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS
        )
    )
    ax.get_xaxis().set_visible(False)
    ...
    plt.title(
        "max uncertainty \n steering angle=%s \n uncertainty=%s"
        % (
            round(online_steering_angles[max_idx_unc], 5),
    ...
    plt.imshow(
        mpimg.imread(center_images[min_idx_cte]).reshape(
            IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS
        )
    )
    ax.get_xaxis().set_visible(False)
    ...
    plt.title(
        "min CTE \n steering angle=%s \n CTE=%s"
        % (
            round(online_steering_angles[min_idx_cte], 5),
    ...
    plt.imshow(
        mpimg.imread(center_images[max_idx_cte]).reshape(
            IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS
        )
    )
    ax.get_xaxis().set_visible(False)
    ...
    plt.title(
        "max CTE \n steering angle=%s \n CTE=%s"
        % (
            round(online_steering_angles[max_idx_cte], 5),
