...
if __name__ == "__main__":
    ...
    encoder_mse = tensorflow.keras.models.load_model(
        "sao/encoder-track3-MSEloss" + WHAT + "-CI-RETRAINED-2X-UNC"
    )
    decoder_mse = tensorflow.keras.models.load_model(
        "sao/decoder-track3-MSEloss" + WHAT + "-CI-RETRAINED-2X-UNC"
    )
    vae_mse = VAE(
    ...
    decoder_vae = tensorflow.keras.models.load_model(
        "sao/decoder-track3-VAEloss" + WHAT + "-CI-RETRAINED-2X-UNC"
    )
    vae_vae = VAE(
        model_name="encoder_vae",
        loss="VAE",
        latent_dim=cfg.SAO_LATENT_DIM,
        encoder=encoder_vae,
        decoder=decoder_vae,
    ...
    for x in tqdm(data_nominal):
        ...
        list_reconstructed_mse.append(
            reconstructed_mse.reshape(
                RESIZED_IMAGE_HEIGHT, RESIZED_IMAGE_WIDTH, IMAGE_CHANNELS
            )
        )
        list_reconstructed_vae.append(
            reconstructed_vae.reshape(
                RESIZED_IMAGE_HEIGHT, RESIZED_IMAGE_WIDTH, IMAGE_CHANNELS
            )
        )
    ...
    plt.hist(
        ...
        bins=50,
        alpha=0.2,
        label="unseen images generated by VAE (MSE loss)",
    )
    plt.hist(
        lpl_var_vae,
        bins=50,
        alpha=0.2,
        label="unseen images generated by VAE (VAE loss)",
    )
