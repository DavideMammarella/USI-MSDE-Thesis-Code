...
INSERT_STRING = (
    "INSERT INTO 'prec_recall' ('anomaly_detector', 'threshold_type', 'threshold', 'true_positives', "
...
class PrecisionRecallAnalysis:
    def __init__(
        ...
        self.prec = self._calculate_precision(
        ...
        self.auroc = auroc
        self.false_positive_rate = self._calc_false_positive_rate(
...
def get_aurocs(db: Database):
    cursor = db.cursor.execute(
        "SELECT anomaly_detector, auroc FROM prec_recall GROUP BY anomaly_detector, auroc"
    )
