...
def calc_uncertainty_score(
    all_uncertainty_objects, start_frame, end_frame_excl, ad_name
) -> float:
    sums = []
    for i in range(start_frame, end_frame_excl - CALC_SCOPE + 1):
        sum = 0
        for j in range(i, i + CALC_SCOPE):
            sum = sum + all_uncertainty_objects[i].uncertainty_of(
                ad_name=ad_name
            )
        sums.append(sum)
    max_sum = max(sums)
...
class Window:
    def __init__(
        ...
        self.uncertainty_score = calc_uncertainty_score(
            all_uncertainty_objects=all_uncertainty_objects,
            start_frame=start_frame,
            end_frame_excl=end_frame,
            ad_name=ad_name,
        )
...
def get_true_positives_count(db: Database, ad_name: str, threshold: float):
    cursor = db.cursor.execute(
        ...
        + "where ad_name = ? and uncertainty_score >= ? and type = 'anomaly'",
        (ad_name, threshold),
    )
    return _unpack_single_result_cursor(cursor)


def get_false_positives_count_ignore_subsequent(
    db: Database, ad_name: str, threshold: float
):
    cursor = db.cursor.execute(
        """select count(*) from windows 
...
def store_single_image_windows(
    setting: int,
    window_id: int,
    window_type: str,
    entries,
    ...
    for ad_name in ads:
        window = Window(
            ...
            window_id=window_id,
            ad_name=ad_name,
            start_frame=start_inc,
            end_frame=end_excl,
            all_uncertainty_objects=entries,
